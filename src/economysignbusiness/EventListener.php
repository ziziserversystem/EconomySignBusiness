...
class EventListener implements Listener
{
    ...
    public function onBreak(BlockBreakEvent $event)
    {
        ...
        if ($tile instanceof Sign) {
            ...
            if (!$player->isOp()) {
            	$player->sendMessage("§a【運営】 §cこの看板を壊す権限がありません");
            	$event->setCancelled();
            	return;
            }
            switch ($line[0]) {
                ...
                case API::EXCHANGE_TAG:
                    ...
                    $player->sendMessage("§a【運営】 §f販売看板を削除しました");
                    break;
            }
        }
    }
    ...
    public function onChange(SignChangeEvent $event)
    {
        ...
        if (!$player->isOp()) {
            $player->sendMessage("§a【運営】 §cこの看板を作成する権限がありません");
            return;
        }
        switch ($line[0]) {
            case "buy":
                ...
                if (!ctype_digit($item[0])) {
                    $player->sendMessage("§a【運営】 §cID(数字)を書き込んでください");
                    return;
                }
                $itemName = Item::get((int)$item[0], (int)$item[1])->getName();
                if (!ctype_digit($line[2])) {
                    $player->sendMessage("§a【運営】 §c数値を書き込んでください");
                    return;
                }
                $amount = (int) $line[2];
                $unit = EconomyAPI::getInstance()->getMonetaryUnit();
                ...
                $player->sendMessage("§a【運営】 §f販売看板を作りました");
                break;
            ...
            case "sell":
                ...
                if (count($item) == 1) $item[1] = 0;
                if ($item[1] == null) {
                    $player->sendMessage("§a【運営】 §cしっかりとID:METAの形で書き込んでください");
                    return;
                }
                $itemName = Item::get((int)$item[0], (int)$item[1])->getName();
                if (!ctype_digit($line[2])) {
                ...
                $event->setLine(3, "§l".$unit.$price);
                $this->getProvider()->setShopDataOfSellAndPurchase($event->getBlock(), $item[0], $item[1], $amount, $price);
                $player->sendMessage("§a【運営】 §f売却看板を作りました");
                break;
            ...
            case "trade":
                ...
                $player->sendMessage("§a【運営】 §fトレード看板を作りました");
                break;
        }
    }
    ...
    {
        $player->sendMessage("§a【運営】 §fもう一度タッチしてください");
